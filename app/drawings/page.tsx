...
export default async function DrawingsPage() {
  ...
  const [{ data: lines }, { data: walls }] = await Promise.all([
    ...
    supabase
      .from("wall_panels")
      .select("id, project_id, wall_id, wall_type, level, lineal_feet, production_line_id, pdf_page_id, sort_order, pdf_pages(page_number, image_url), projects(id, name, code), production_lines(name), wall_notes(markup_data)")
      .order("sort_order")
      .order("wall_id")
  ]);
